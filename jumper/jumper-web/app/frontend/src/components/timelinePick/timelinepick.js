...
import $ from "jquery"
import d3 from "d3"
import "../lib/utils/createjs"
import "../lib/utils/TweenMax"
...
export default class TimelinePick {
    ...
    create() {
        this.stageBg, this.stageSpan, this.leftArrow, this.rightArrow,this.interval, this.tempTimer = 0;
        this.tempInterval= 0;
        this.init();
        if (!this.nowTime) {
        ...
        this.paddingX = 30 ;
        this.xScale = d3.scale.linear().domain([0, this.options.number]).range([0, this.stage.xp - (this.paddingX) ]);
     ...
     */
    checkingBound() {
        return this.nowTime >= this.options.threshold
    }
    ...
    caculateTimeSpan(){
        ...
        // ms (默认)
        var baseTime = drawUtils.computeTime('minute', me.nowTime, this.interval, -me.options.number);
        startTime = drawUtils.computeTime('minute', baseTime, this.interval, me.options.spanIndex.start);
        endTime = drawUtils.computeTime('minute', baseTime, this.interval, me.options.spanIndex.end);
        return {
...
const drawUtils = {
    ...
    // 时间格式化
    computeTime(type, time, interval, plus) {
