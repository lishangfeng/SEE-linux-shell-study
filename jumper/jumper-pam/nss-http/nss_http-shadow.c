...
enum nss_status _nss_http_setspent_locked(int stayopen)
{
    json_t *json_root;
    json_root = nss_request("shadow", "", "");
...
enum nss_status _nss_http_endspent_locked(void)
{
    if (ent_json_root){
...
enum nss_status _nss_http_getspent_r_locked(struct spwd *result, char *buffer, size_t buflen, int *errnop)
{
...
enum nss_status _nss_http_getspnam_r_locked(const char *name, struct spwd *result, char *buffer, size_t buflen, int *errnop)
{
    json_t *json_root;

    json_root = nss_request("shadow", "name", name);
